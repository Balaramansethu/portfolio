import React from "react";
import "../ContactMe/ContactMe.css";
const ContactMe = () => {
  return (
    <div className="min-h-screen py-20 jetbrains-mono bg-gray-950 text-white">
      <div class="max-w-6xl mx-auto px-4">
        <div className="border rounded-lg overflow-hidden bg-[#0d1117] border-[#30363d] opacity-100">
          <div className="p-4 flex items-center gap-2 border-b bg-[#161b22] border-[#30363d]">
            <div className="w-3 h-3 rounded-full bg-[#ff5f57]"></div>
            <div className="w-3 h-3 rounded-full bg-[#febc2e]"></div>
            <div className="w-3 h-3 rounded-full bg-[#28c841]"></div>
            <span className="text-sm ml-2 text-[#8b949e]">contact.sh</span>
          </div>
          <div className="p-6 space-y-6 ">
            <div className="flex items-center gap-2 text-lg opacity-100">
              <span className="text-green-500">→</span>
              <span className="text-[#e6edf3]">./contact-me</span>
              <span className="w-2 h-5 bg-blue-400 animate-pulse"></span>
            </div>
            <div className="space-y-1">
              <div className="text-blue-500 text-lg"># Let's Connect!</div>
              <div className="text-base text-[#8b949e]">
                "Currently building web applications and exploring new technologies. Open to discussing interesting projects and opportunities"
              </div>
            </div>
            <div className="space-y-4 pt-4">
              <div className="text-base text-[#e6edf3]">Available on:</div>
              <div className="space-y-3 pl-4">
                <div className="group">
                  <a
                    href="https://github.com/Balaramansethu"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="inline-flex items-center gap-4 text-base transition-colors
                                                        text-[#8b949e] hover:text-blue-400"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      className="lucide lucide-github w-5 h-5"
                    >
                      <path d="M15 22v-4a4.8 4.8 0 0 0-1-3.5c3 0 6-2 6-5.5.08-1.25-.27-2.48-1-3.5.28-1.15.28-2.35 0-3.5 0 0-1 0-3 1.5-2.64-.5-5.36-.5-8 0C6 2 5 2 5 2c-.3 1.15-.3 2.35 0 3.5A5.403 5.403 0 0 0 4 9c0 3.5 3 5.5 6 5.5-.39.49-.68 1.05-.85 1.65-.17.6-.22 1.23-.15 1.85v4"></path>
                      <path d="M9 18c-4.51 2-5-2-7-2"></path>
                    </svg>
                    <span class="group-hover:no-underline">Balaramansethu</span>
                  </a>
                </div>
                <div class="group">
                  <a
                    href="https://www.linkedin.com/in/balaraman-s-abb94422a/"
                    target="_blank"
                    rel="noopener noreferrer"
                    class="inline-flex items-center gap-4 text-base transition-colors text-[#8b949e] hover:text-blue-400"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      class="lucide lucide-linkedin w-5 h-5"
                    >
                      <path d="M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z"></path>
                      <rect width="4" height="12" x="2" y="9"></rect>
                      <circle cx="4" cy="4" r="2"></circle>
                    </svg>
                    <span class="group-hover:no-underline">Balaraman</span>
                  </a>
                </div>
                <div class="group">
                  <a
                    href="mailto:baalaaraman@gmail.com"
                    class="inline-flex items-center gap-4 text-base transition-colors text-[#8b949e] hover:text-blue-400"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="24"
                      height="24"
                      viewBox="0 0 24 24"
                      fill="none"
                      stroke="currentColor"
                      stroke-width="2"
                      stroke-linecap="round"
                      stroke-linejoin="round"
                      class="lucide lucide-mail w-5 h-5"
                    >
                      <rect width="20" height="16" x="2" y="4" rx="2"></rect>
                      <path d="m22 7-8.97 5.7a1.94 1.94 0 0 1-2.06 0L2 7"></path>
                    </svg>
                    <span class="group-hover:no-underline">
                      baalaaraman@gmail.com
                    </span>
                  </a>
                </div>
              </div>
            </div>
            <div class="flex items-center gap-2 text-lg pt-4 opacity-100">
              <span class="text-green-500">→</span>
              <span class="text-[#e6edf3]">_</span>
              <span class="w-2 h-5 bg-blue-400 animate-pulse"></span>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default ContactMe;
